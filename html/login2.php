                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    if(mysqli_stmt_fetch($stmt)){
                        if(password_verify($password, $hashed_password)){
                            ...
                            if($role == "Human Resources" || $role == "All"){
                                session_start();
                                ...
                                header("location: HumanData.php");
                            }
                            else{
                                echo "You are not authorized to view this form.";
                            }
                        } 
                        else{
...
<body>
    <div class="wrapper"> 
        <h2>Human Resource Data Search Form Login</h2>
        <p>Please fill in your credentials to login.</p>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            <div class="form-group <?php echo (!empty($username_err)) ? 'has-error' : ''; ?>">
                <label>Username</label>
                <input type="text" name="username" class="form-control" value="<?php echo $username; ?>" maxlength="20">
